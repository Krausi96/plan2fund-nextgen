 ...
 * Used by: External scraper tools (not web app)
 * 
 * What it does:
 * - Parses HTML/text from funding program web pages
 * - Uses LLM to extract structured requirements and metadata
 * - Returns categorized requirements and program metadata
 * 
 * Note: This is used by separate scraper tools, not the web app directly
 */
...
NOTE: Do NOT extract fields that are already in metadata (location, company_type, company_stage, deadline, open_deadline, program_focus, co_financing, use_of_funds, impact_focus) - these are handled separately.

1. Eligibility: industry_restriction, eligibility_criteria (NOT company_type, company_stage - those go in metadata)
2. Documents: required_documents, document_type, format
3. Financial: repayment_terms, interest_rate, equity_terms, funding_rate, grant_ratio, guarantee_fee, guarantee_ratio (NOT co_financing - that goes in metadata)
4. Technical: technical_requirement, trl_level
5. Legal: legal_requirement
6. Timeline: duration, application_window (NOT deadline, open_deadline - those go in metadata)
7. Team: team_size, team_composition
8. Compliance: compliance_requirement
9. Application: application_process, evaluation_criteria, application_form, application_requirement
10. Funding_Details: capex_opex, revenue_model, market_size, project_details (NOT use_of_funds - that goes in metadata)
11. Restrictions: restrictions, intellectual_property, consortium, diversity
12. Terms: success_metrics, trl_level
...
function transformLLMResponse(
  ...
  // Fields that should NOT be extracted as requirements (they go in metadata only)
  const metadataOnlyFields = [
    'company_type', 'company_stage', 'location', 'geographic_eligibility',
    'deadline', 'open_deadline', 'program_focus', 'innovation_focus', 
    'technology_area', 'research_domain', 'sector_focus',
    'co_financing', 'use_of_funds',
    'environmental_impact', 'social_impact', 'economic_impact', 'innovation_impact', 'impact_focus'
  ];
    ...
    Object.entries(llmResponse.requirements).forEach(([category, items]: [string, any]) => {
      if (Array.isArray(items)) {
        items.forEach((item: any) => {
          ...
          // Skip metadata-only fields (these should not be in requirements)
          if (metadataOnlyFields.includes(type.toLowerCase())) {
            return;
          }
  ...
  // Note: company_type and company_stage should be in metadata, not in requirements
  // They are extracted separately in the metadata section
