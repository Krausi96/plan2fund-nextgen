...
type ProductSelectionProps = {
  ...
  pendingProduct?: ProductType | null;
...
export default function ProductSelection({
  ...
  // Determine selected meta (use pendingProduct if available, otherwise use selectedProductMeta or find from options)
  const selectedMeta = selectedProductMeta ?? productOptions?.find((option) => option.value === (pendingProduct || productType)) ?? productOptions?.[0] ?? null;
        ...
        >
          <ul className="flex flex-col gap-1">
            {productOptions?.map((option) => {
              const isActive = option.value === (pendingProduct || productType);
              return (
                <li key={option.value}>
                  <button
                    type="button"
