...
import type { Program, FundingProfile } from '../../core/types';
  ...
  for (const program of programs) {
    ...
    if (matchResult.overall >= minScore) {
      results.push({
        ...
        matchedCriteria: matchResult.breakdown
          .filter(b => b.score >= 70)
          .map(b => b.criteria),
        gaps: matchResult.breakdown
          .filter(b => b.score < 50)
          .map(b => b.criteria),
...
function generateRecommendations(matchResult: MatchScore): string[] {
  ...
  const lowScores = matchResult.breakdown.filter(b => b.score < 50);
