...
// CI Coverage Check Script - Lenient Version for Development
...
console.log('🔍 COVERAGE CHECK - Programs × Fields × Questions (Lenient Mode)\n');
...
// Identify critical gaps (fields with <30% coverage - more lenient)
const criticalGaps = gaps.filter(gap => {
  const fieldCoverageCount = fieldCoverage[gap.field];
  return fieldCoverageCount < totalPrograms * 0.3; // Lowered from 0.5 to 0.3
});
...
if (criticalGaps.length === 0) {
  ...
  console.log(`Fields with <30% coverage: ${criticalFields.join(', ')}`);
...
// Validation with lenient thresholds
const errors = [];

if (coveragePercentage < 40) { // Lowered from 70% to 40%
  errors.push(`Overall coverage ${coveragePercentage}% is below minimum 40%`);
}
...
for (const [field, count] of Object.entries(fieldCoverage)) {
  ...
  if (coveragePercent < 20) { // Lowered from 50% to 20%
    errors.push(`Field ${field} has only ${coveragePercent}% coverage (minimum 20%)`);
  }
}
...
if (errors.length === 0) {
  ...
  console.log('📈 Coverage is improving - continue adding program overlays');
  process.exit(0);
} else {
  console.log('❌ Coverage validation failed:');
  errors.forEach(error => console.log(`  - ${error}`));
  console.log('\n💡 Next steps:');
  console.log('  1. Add more overlays to programs.json');
  console.log('  2. Focus on fields with <20% coverage');
  console.log('  3. Run "npm run coverage" for strict mode');
  process.exit(1);
}
