#!/usr/bin/env python3
"""
Folder Restructure Plan - Simple, safe reorganization
Creates a logical folder structure without overengineering
"""
...
def create_restructure_plan():
    """Create a safe folder restructure plan"""
    ...
    plan = {
        ...
        "proposed_structure": {
            "pages": {
                "marketing/": [
                    "index.tsx", "about.tsx", "pricing.tsx", "contact.tsx", "faq.tsx", "for.tsx"
                ],
                "user-flow/": [
                    "reco.tsx", "results.tsx", "editor.tsx", "optimized-editor.tsx", "preview.tsx"
                ],
                "checkout/": [
                    "checkout.tsx", "confirm.tsx", "thank-you.tsx"
                ],
                "legal/": [
                    "privacy.tsx", "terms.tsx", "legal.tsx", "privacy-settings.tsx"
                ],
                "admin/": [
                    "dashboard.tsx", "library.tsx", "export.tsx", "advanced-search.tsx"
                ]
            },
            "src/lib": {
                "api/": [
                    "apiClient.ts", "endpoints.ts", "airtable.ts", "database.ts"
                ],
                "ai/": [
                    "aiService.ts", "aiHelper.ts", "aiHelperGuardrails.ts", "aiChipParser.ts",
                    "enhancedRecoEngine.ts", "recommendationEngine.ts"
                ],
                "data/": [
                    "dataSource.ts", "programs.ts", "questions.ts", "sourceRegister.ts",
                    "ScrapedProgram.ts", "libraryExtractor.ts"
                ],
                "export/": [
                    "export.ts", "comprehensiveExport.ts", "submissionPack.ts", "templates.ts"
                ],
                "editor/": [
                    "editorTemplates.ts", "programTemplates.ts", "requirementsExtractor.ts",
                    "requirementsMapper.ts", "intakeParser.ts", "prefill.ts"
                ],
                "recommendation/": [
                    "decisionTree.ts", "decisionTreeParser.ts", "dynamicDecisionTree.ts",
                    "dynamicQuestionEngine.ts", "enhancedDecisionTree.ts", "doctorDiagnostic.ts",
                    "advancedSearchDoctor.ts", "targetGroupDetection.ts"
                ],
                "utils/": [
                    "utils.ts", "validation.ts", "formatting.ts", "constants.ts",
                    "motion.ts", "theme.ts", "seo.ts", "translationValidator.ts"
                ],
                "business/": [
                    "pricing.ts", "payments.ts", "financialCalculator.ts", "planStore.ts",
                    "multiUserDataManager.ts", "teamManagement.ts", "addons.ts"
                ],
                "scraper/": [
                    "webScraperService.ts", "enhancedDataPipeline.ts"
                ]
            }
        },
        "migration_steps": [
...
def main():
    print("📁 CREATING FOLDER RESTRUCTURE PLAN")
    print("=" * 50)
    
    plan = create_restructure_plan()
    
    # Save plan
    with open("folder_restructure_plan.json", "w") as f:
        json.dump(plan, f, indent=2)
    
    print("✅ Restructure plan created!")
    print(f"📊 Will organize {len(plan['proposed_structure']['pages'])} page categories")
    print(f"📊 Will organize {len(plan['proposed_structure']['src/lib'])} lib categories")
    print(f"📄 Plan saved to: folder_restructure_plan.json")
    print("\n🎯 NEXT STEPS:")
    print("1. Review the plan")
    print("2. Create backup")
    print("3. Execute migration steps")
