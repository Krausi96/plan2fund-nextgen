...
// Detailed Rule Traces - Shows H/S/U → % → Bullets for 3 personas
...
console.log('🔍 DETAILED RULE TRACES - H/S/U → % → Bullets\n');

// Persona definitions with program IDs
const personas = {
  B2C_FOUNDER: {
    name: 'B2C Founder',
    answers: {
...
function traceProgram(answers, program) {
  ...
  // Generate bullets based on program and rules
  if (hardPassed > 0) {
  ...
  if (program.thresholds && program.thresholds.funding_rate) {
    bullets.push(`${program.thresholds.funding_rate} funding rate`);
  }
