...
export class AIHelperGuardrails {
  ...
  private extractChips(input: string): Chip[] {
    ...
    missingFields.forEach(field => {
      ...
      if (defaults[field as keyof typeof defaults]) {
        chips.push({
          field,
          label: field.charAt(0).toUpperCase() + field.slice(1).replace('_', ' '),
          value: defaults[field as keyof typeof defaults].value,
          confidence: defaults[field as keyof typeof defaults].confidence,
          editable: true
        });
      }
    });
