...
import { analyzeFreeTextEnhanced, scoreProgramsEnhanced } from "@/lib/enhancedRecoEngine";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Use enhanced free text analysis
    const analyzed = featureFlags.isEnabled('AI_ENABLED') ? await analyzeFreeTextEnhanced(description) : { normalized: {} };
    const normalized = analyzed.normalized;
    const scored = await scoreProgramsEnhanced(normalized, mode);
