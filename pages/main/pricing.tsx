...
export default function Pricing() {
  const { t } = useI18n();
  const [selectedRoute, setSelectedRoute] = useState<'grant'|'bank'|'equity'|'visa'|'ams'>('grant');
  const [addonPack, setAddonPack] = useState(false);
        ...
        <section className="py-8 bg-white border-b">
          <div className="container mx-auto px-4">
            <div className="flex flex-wrap gap-4 justify-center">
              {Object.entries(routeNames).map(([key, name]) => (
                <button
                  key={key}
              ...
              <div className="bg-blue-50 rounded-2xl p-8 border-2 border-blue-200">
                <div className="flex items-start gap-4">
                  <input
                    ...
                    id="addonPack"
                    checked={addonPack}
                    onChange={(e) => setAddonPack(e.target.checked)}
                    className="mt-1 w-5 h-5 text-blue-600"
                  />
                  <div className="flex-1">
                    <label htmlFor="addonPack" className="text-xl font-bold text-gray-900 cursor-pointer">
                      Add-on Pack (+€39)
                    </label>
                    <p className="text-gray-600 mt-2">
                      Rush to first draft, one extra revision included, provider form help
                    </p>
                    <p className="text-sm text-gray-500 mt-1">
